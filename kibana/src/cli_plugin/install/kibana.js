...
async function rebuildCache(settings, logger) {
  ...
  const kibanaArgs = [(0, _utils.fromRoot)('./src/cli'), '--env.name=production', '--optimize.useBundleCache=false', '--server.autoListen=false', '--plugins.initialize=false', '--uiSettings.enabled=false'];
